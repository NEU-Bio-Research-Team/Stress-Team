...
CFG_PATH="config/phase2_canonical_config.json"
CFG_LABEL="phase2_canonical_tuned_legacy"
...
echo "=== PHASE 2 CANONICAL TUNED LEGACY PARALLEL RUN ==="
...
python3 - "$RAW_DIR" "$OUT_ROOT" "$TOTAL_RUNS" "$CFG_PATH" <<'PY'
import json
import sys
...
merged_csv = out_root / "lob_full_simulation_llm_tuned_legacy.csv"
summary_json = out_root / "lob_full_summary_llm_tuned_legacy.json"
merged.to_csv(merged_csv, index=False)

summary = {
    "config_json": cfg_path,
    "known_limitations_path": "KNOWN_LIMITATIONS.md",
